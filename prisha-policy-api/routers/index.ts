...
export const appRouter  = t.router({
    ...
    .query(async ({ input }) => {
      ...
      try {
        const userRes = await client.query(
          `SELECT u.user_id, u.email, u.password, e.role, e.name, e.employee_id
           FROM users u
        ...
        // TODO: Replace this password check with a secure method (e.g., hashing)
  ...
  .mutation(async ({ input }) => {
   ...
   deleteDependents : t.procedure
  .input(z.object({
    dependentId: z.number(),
  }))
  .mutation(async ({ input }) => {
    const { dependentId } = input;
    try {
      const res = await client.query(
        'DELETE FROM dependents WHERE dependent_id = $1 RETURNING *',
        [dependentId]
      );
